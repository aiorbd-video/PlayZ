...
export default function Home() {
  ...
  // ✅ আপনার ডাটা ফেচিং লিংকগুলো এখানে সঠিকভাবে ফাংশনের ভেতর সেট করা হলো
    // ✅ আপনার ডাটা ফেচিং লিংকগুলো এখানে সঠিকভাবে ফাংশনের ভেতর সেট করা হলো
  const { data: rawMatches } = useSWR('https://www.ratulxlive.duckdns.org/api/proxy?type=matches', fetcher, { refreshInterval: 90000, revalidateOnFocus: false, dedupingInterval: 15000 });
  
  const { data: channelData } = useSWR('https://www.ratulxlive.duckdns.org/api/channels', fetcher, { refreshInterval: 60000, revalidateOnFocus: false, revalidateOnReconnect: true, dedupingInterval: 20000 });
  
  const { data: m3uData } = useSWR('https://www.ratulxlive.duckdns.org/api/m3u', fetcher, { refreshInterval: 90000, revalidateOnFocus: false, dedupingInterval: 30000 });
  const { data } = useSWR('https://www.ratulxlive.duckdns.org/api/notice', fetcher, { refreshInterval: 30000, revalidateOnFocus: false });
